# Track Studio API 速览（指纹 + 精选艺人，对接版）
...
- 认证：需要 API 密钥 + `userKey`（body）
- 适用场景：同步 Track Studio 客户端“用户喜欢的艺人”轻量数据；数据量远小于全量指纹，服务端直接按全量快照做归一化与合并
...
|---|---|---|---|---|---|
| userKey | body | string | 是 | UUID v4 | 同步用户标识 |
| artists | body | object[] | 是 | 0..5000 | 客户端精选艺人快照 |
| artists[].name | body | string | 是 | 去空白后非空，≤ 200 字符 | 艺人名 |
| artists[].count | body | number | 否 | > 0 | 当前艺人累计次数 |
| artists[].fingerprints | body | string[] | 否 | 每项 64 位十六进制 SHA256 | 贡献该艺人计数的原始指纹集合 |
| count | body | integer | 否 | >= 0 | 客户端声明的归一化艺人数；若提供，必须与服务端归一化结果一致 |
| hash | body | string | 否 | 64 位十六进制 SHA256 | 客户端声明的快照哈希；若提供，必须与服务端归一化结果一致 |
...
|---|---|---|
| success | boolean | 是否成功 |
| needSync | boolean | 客户端提交前是否与服务端存在差异 |
| changed | boolean | 本次请求是否写入并更新了服务端快照 |
| reason | string | `already_synced` / `server_empty` / `client_empty` / `merged` / `client_outdated` |
| message | string | 友好提示 |
| clientSnapshot | object | 客户端归一化后的快照统计与数据 |
| serverSnapshotBefore | object | 合并前服务端快照统计与数据 |
| mergedSnapshot | object | 合并后的权威快照；客户端应使用它覆盖本地 |
| performance | object | 性能指标 |
| timestamp | string | 服务端时间戳 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| artistCount | number | 艺人数 |
| totalCount | number | 所有艺人 `count` 之和 |
| fingerprintCount | number | 所有关联原始指纹数量之和 |
| hash | string | 归一化快照哈希 |
| lastSyncAt | string/null | 上次服务端同步时间（仅服务端快照返回） |
| items | object[] | 快照明细：`{ name, count, fingerprints }` |

- 合并规则（实现口径）：
  - 先按归一化艺人名合并。
  - `fingerprints` 做并集去重。
  - `count` 取 `max(已有count, 新count, fingerprints.length)`。
